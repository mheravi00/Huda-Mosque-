...
export const studentsApi = { ...collection('/api/students'), admin: collection('/api/admin/students'), guardians: id => api.get(`/api/students/${id}/guardians`), linkGuardian: (id, guardian_id) => api.post(`/api/admin/students/${id}/guardians`, { guardian_id }), unlinkGuardian: (id, guardianId) => api.delete(`/api/admin/students/${id}/guardians/${guardianId}`), classes: id => api.get(`/api/admin/students/${id}/classes`), assignClass: (id, class_id, override_reason) => api.post(`/api/admin/students/${id}/classes`, override_reason ? { class_id, override_reason } : { class_id }), removeClass: (id, classId) => api.delete(`/api/admin/students/${id}/classes/${classId}`), bracketReview: () => api.get('/api/admin/students/bracket-review') };
...
export const messageTemplatesApi = { ...collection('/api/message-templates'), admin: collection('/api/admin/message-templates') };
...
export const notesApi = { access: () => api.get('/api/notes/access'), list: studentId => api.get(`/api/students/${studentId}/notes`), create: (studentId, body) => api.post(`/api/students/${studentId}/notes`, body), update: (id, body) => api.patch(`/api/student-notes/${id}`, body), remove: id => api.delete(`/api/student-notes/${id}`) };
